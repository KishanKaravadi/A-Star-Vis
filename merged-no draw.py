...
# Spot object to populate the grid. Allows the coloring of the grid cells and to give each cell properties such as fire easily.
class Spot:
    ...
    def update_buffer_neighbors(self, grid):
        self.buffer_neighbors = []
        # DOWN
        if self.row < self.total_rows - 1 and not (grid[self.row + 1][self.col].is_barrier() or grid[self.row + 1][self.col].is_fire() or grid[self.row+1][self.col].adj_fire(grid)):
            self.buffer_neighbors.append(grid[self.row + 1][self.col])
        # UP
        if self.row > 0 and not (grid[self.row - 1][self.col].is_barrier() or grid[self.row - 1][self.col].is_fire() or grid[self.row - 1][self.col].adj_fire(grid)):
            self.buffer_neighbors.append(grid[self.row - 1][self.col])
        # RIGHT
        if self.col < self.total_rows - 1 and not (grid[self.row][self.col + 1].is_barrier() or grid[self.row][self.col + 1].is_fire() or grid[self.row][self.col + 1].adj_fire(grid)):
            self.buffer_neighbors.append(grid[self.row][self.col + 1])
        # LEFT
        if self.col > 0 and not (grid[self.row][self.col - 1].is_barrier() or grid[self.row][self.col - 1].is_fire() or grid[self.row][self.col - 1].adj_fire(grid)):
            self.buffer_neighbors.append(grid[self.row][self.col - 1])
...
# Allows the reconstruction of the path thru a hashmap of the parent cells
...
# A* Star Algorithm which is basically the dijkstra's written in class but with an added heuristic.
...
# Same algorithm as above but added the fire's neighbors as information to avoid as well. If there is no paths that have a buffer, it calls the older algorithm and runs that.
def buffer_algorithm(draw, grid, start, end):
    count = 0
    ...
    while not open_set.empty():
        ...
        for neighbor in current.buffer_neighbors:
            temp_g_score = g_score[current] + 1
...
# function to get the positon of the GUI clicked to change colors
...
# Makes a 2D Array of Spot objects for the grid
...
# Makes the ship with the given specifications
...
# Draws the grid lines onto the GUI for pygame
...
# Bot 1 that ignores fire cells completely


def Bot1(win, width, ROWS, Q):
    ...
    while run:
        ...
        if (time):
            ...
            for row in grid:
                for spot in row:
                    if (spot.is_white() or spot.is_start() or spot.is_end()):
                        K = 0
...
# Bot 2 that avoids the current fire cells on path
def Bot2(win, width, ROWS, Q):
    grid = make_grid(ROWS, width)
    random_bot, random_button, random_fire = make_ship(
        lambda: draw(win, grid, ROWS, width), grid, ROWS)
...
# Bot 3 that avoids current fire cells on path and surrounding neighbors to that fire


def Bot3(win, width, ROWS, Q):
    grid = make_grid(ROWS, width)
    random_bot, random_button, random_fire = make_ship(
        lambda: draw(win, grid, ROWS, width), grid, ROWS)
    ...
    for row in grid:
        for spot in row:
            spot.update_neighbors(grid)
            spot.update_buffer_neighbors(grid)
            spot.update_unres_neighbors(grid)
    ...
    # spot.update_unres_neighbors(grid)
    a = buffer_algorithm(lambda: draw(win, grid, ROWS, width),
                         grid, start, end)
...
# Bot 4 that uses the distance of the fire to the neighbor to choose the path away from the fire.


def Bot4(win, width, ROWS, Q):
    grid = make_grid(ROWS, width)
    random_bot, random_button, random_fire = make_ship(
        lambda: draw(win, grid, ROWS, width), grid, ROWS)
...
# Code that was written to collect and aggregate the number of successes for each bot
...
def main(win, width):
    ...
    for i in range(0, 11):
        for _ in range(51):
            success[i/10] += Bot4(win, width,  ROWS, i/10)
    print(success)
